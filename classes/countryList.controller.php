...
class DpdPolandCountryListController extends DpdPolandController
{
	...
	private function init()
	{
		if (Tools::getValue('disable_country') && $id_country = Tools::getValue('id_country'))
			if ($this->changeEnabled((int)$id_country, true))
				...
				$this->module_instance->displayError($this->module_instance->l('Could not change country status', FILENAME));
		...
		if (Tools::isSubmit('disableCountries'))
		{
			...
			else
			{
				$this->module_instance->outputHTML(
					$this->module_instance->displayError($this->module_instance->l('No selected countries', self::FILENAME))
				);
			}
		}
	...
	private function displaySuccessStatusChangingMessage()
	{
		...
		DpdPoland::addFlashMessage($this->module_instance->l('Country status changed successfully', self::FILENAME));
		Tools::redirectAdmin($this->module_instance->module_url.'&menu=country_list&pagination='.$selected_pagination.'&CountryOrderBy='.$order_by.'&CountryOrderWay='.$order_way.'&submitFilterCountries='.$page);
	}
	...
	private function changeEnabledMultipleCountries($countries = array(), $disable = false)
	{
		foreach ($countries as $id_country)
			if (!$this->changeEnabled((int)$id_country, $disable))
				self::$errors[] = sprintf($this->module_instance->l('Could not change country status, ID: %s', self::FILENAME), $id_country);
		...
		else
		{
			...
			DpdPoland::addFlashMessage($this->module_instance->l('Selected countries statuses changed successfully', self::FILENAME));
			Tools::redirectAdmin($this->module_instance->module_url.'&menu=country_list&pagination='.$selected_pagination.'&CountryOrderBy='.$order_by.'&CountryOrderWay='.$order_way.'&submitFilterCountries='.$page);
		}
	}
	...
	public static function disableDefaultCountries()
	{
		...
		if (version_compare(_PS_VERSION_, '1.5', '<'))
		{
			foreach (Country::getCountries((int)$context->language->id) as $country)
				if (!in_array($country['iso_code'], DpdPolandCountry::$default_enabled_countries) && !DpdPolandCountry::addCountry($country['id_country'], (int)$context->shop->id, 0))
					return false;
		}
		else
			foreach (array_keys(Shop::getShops()) as $id_shop)
				foreach (Country::getCountriesByIdShop($id_shop, Configuration::get('PS_LANG_DEFAULT')) as $country)
					if (!in_array($country['iso_code'], DpdPolandCountry::$default_enabled_countries) && !DpdPolandCountry::addCountry($country['id_country'], (int)$id_shop, 0))
						return false;
